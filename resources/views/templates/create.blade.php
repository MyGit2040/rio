            ...
            <form method="POST" action="{{ route('templates.store') }}" class="space-y-5">
                ...
                <x-card title="Message type">
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                        @foreach ([['text','Text','doc'], ['media','Image / Video / File','tag'], ['poll','Poll','grid'], ['buttons','Buttons','send'], ['carousel','Carousel','grid']] as [$val, $title, $ic])
                ...
                <x-card title="Content">
                    <div class="space-y-4">
                        ...
                        <div x-show="type === 'poll'" x-cloak class="space-y-3">
                            ...
                            <label class="inline-flex items-center gap-2">
                                <input type="checkbox" name="poll_multiple" value="1" class="rounded border-gray-300 text-green-600 focus:ring-green-500">
                    ...
                    <div class="mt-6 flex items-center gap-3">
                        <x-btn type="submit" variant="primary">Save template</x-btn>
    ...
    <script>
        function templateEditor() {
            return {
                ...
                name: @js(old('name', '')),
                body: @js(old('body', '')),
                mediaType: @js(old('media_type', 'image')),
                mediaUrl: @js(old('media_url', '')),
                pollQuestion: @js(old('poll_question', '')),
                options: @js(old('poll_options', ['', ''])),
                variants: @js(old('variants', [])),
                pollMediaUrl: @js(old('poll_media_url', '')),
                pollMediaType: @js(old('poll_media_type', 'image')),
                buttonsTitle: @js(old('buttons_title', '')),
                buttonsFooter: @js(old('buttons_footer', '')),
                buttons: @js(old('buttons', [['type' => 'reply', 'text' => '', 'value' => '']])),
                cards: @js(old('cards', [['image' => '', 'title' => '', 'body' => '', 'buttons' => []]])),
                variantCount: 10,
